...
class RegistroAquisicao extends StatefulWidget {
  final String token;

  const RegistroAquisicao({super.key, required this.token});

  @override
  _RegistroAquisicaoState createState() => _RegistroAquisicaoState();
}

class _RegistroAquisicaoState extends State<RegistroAquisicao> {
        ...
        if (decodedResponse is List) {
          setState(() {
            _atividades = decodedResponse
                .where((atividade) =>
                    atividade['tipo'] ==
                    'Registro de Aquisição ou Produção Própria de Insumo')
                .toList();
            _isLoading = false;
          });
